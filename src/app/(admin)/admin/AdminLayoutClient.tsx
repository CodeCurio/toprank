...
export default function AdminLayoutClient({ children, session }: { children: ReactNode, session: any }) {
  const pathname = usePathname();

  // Check if current route is an editor (Post or Portfolio)
  const isEditor = 
    pathname.includes('/admin/post/new') || 
    pathname.match(/\/admin\/post\/[^/]+$/) ||
    pathname.includes('/admin/portfolio/new') ||
            ...
            <nav className="space-y-1">
              <Link
                ...
                className={`flex items-center px-3 py-2.5 text-sm font-medium rounded-xl transition-all group ${
                  pathname === "/admin" ? "bg-blue-600/10 text-blue-400 font-semibold" : "hover:bg-slate-800 hover:text-white"
                }`}
              >
                <LayoutDashboard className={`w-4 h-4 mr-3 ${pathname === "/admin" ? "" : "opacity-60 group-hover:opacity-100"}`} />
                Dashboard
              </Link>
              <Link
                href="/admin/post/new"
              ...
              <Link
                ...
                className={`flex items-center px-3 py-2.5 text-sm font-medium rounded-xl transition-all group ${
                  pathname.includes("/admin/categories") ? "bg-blue-600/10 text-blue-400 font-semibold" : "hover:bg-slate-800 hover:text-white"
                }`}
              >
                <Tags className={`w-4 h-4 mr-3 ${pathname.includes("/admin/categories") ? "" : "opacity-60 group-hover:opacity-100"}`} />
                Categories & Tags
              </Link>
              <Link
                href="/admin/portfolio"
                className={`flex items-center px-3 py-2.5 text-sm font-medium rounded-xl transition-all group ${
                  pathname.includes("/admin/portfolio") ? "bg-blue-600/10 text-blue-400 font-semibold" : "hover:bg-slate-800 hover:text-white"
                }`}
              >
                <LayoutDashboard className={`w-4 h-4 mr-3 ${pathname.includes("/admin/portfolio") ? "" : "opacity-60 group-hover:opacity-100"}`} />
                Portfolio Projects
              </Link>
              <Link
                href="/admin/comments"
                className={`flex items-center px-3 py-2.5 text-sm font-medium rounded-xl transition-all group ${
                  pathname.includes("/admin/comments") ? "bg-blue-600/10 text-blue-400 font-semibold" : "hover:bg-slate-800 hover:text-white"
                }`}
              >
                <MessageSquare className={`w-4 h-4 mr-3 ${pathname.includes("/admin/comments") ? "" : "opacity-60 group-hover:opacity-100"}`} />
                User Comments
              </Link>
              <Link
                href="/admin/settings"
                className={`flex items-center px-3 py-2.5 text-sm font-medium rounded-xl transition-all group ${
                  pathname.includes("/admin/settings") ? "bg-blue-600/10 text-blue-400 font-semibold" : "hover:bg-slate-800 hover:text-white"
                }`}
              >
                <Settings className={`w-4 h-4 mr-3 ${pathname.includes("/admin/settings") ? "" : "opacity-60 group-hover:opacity-100"}`} />
                Admin Settings
              </Link>
            </nav>
          </div>
